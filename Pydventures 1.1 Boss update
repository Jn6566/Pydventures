...
lista_monstros = [
    ...
    {"monstro_indice": 8005, "monstro": "Dragão", "monstro_poder": 25},
        ...
        if armadura_retirada:
            ...
            while True:
                if equipar_armadura == "y":
                    ...
                    # Vê se o jogador atinge as condições para ir para a boss fight:
                    if player_nivel == 9:
                        # Executa a função sala_boss
                        sala_boss(player_nome, player_nivel, player_armadura, player_arma, inventario)
...
""" ############### Combate de Chefe ############### """
def boss_fight(player_nome, player_nivel, player_armadura, player_arma, player_poder, inventario, lista_monstros):
    # Escolhe o boss do banco de dados:
    monstro_encontrado = next((mi for mi in lista_monstros if mi["monstro_indice"] == 8005), None)
    ...
    print("\n\n######################## Combate de Chefe ########################\n")
    print(f"Ao empurrar o portão, {player_nome} se depara com um {monstro_encontrado["monstro"]} em meio a uma")
    print(f"enorme pilha de tesouros. O {monstro_encontrado["monstro"]} dá um grande rugido e começa a voar em")
    print(f"sua direção.\n")
    print(f"Monstro: {monstro_encontrado["monstro"]}, Poder: {monstro_encontrado["monstro_poder"]}\n")
    print("1. Consultar Status do Jogador:\n")
    print("2. Lutar:\n")
    ...
    # Escolhe a próxima ação:
    while True:
        escolha_boss_fight = input("\nEscolha uma opcao: ")
        acao_boss_fight = opcoes_boss_fight.get(escolha_boss_fight)
...
""" ############### Sala de Chefe ############### """
def sala_boss(player_nome, player_nivel, player_armadura, player_arma, inventario):
    # Atualiza status do jogador
    player_poder = player_nivel + player_armadura["armadura_poder"] + player_arma["arma_poder"]
    ...
    opcoes_sala_boss = {
        "1": lambda: player(player_nome, player_nivel, player_armadura, player_arma, player_poder),
        "2": lambda: boss_fight(player_nome, player_nivel, player_armadura, player_arma, player_poder, inventario, lista_monstros),
        "3": lambda: ver_inventario(inventario)
    }
    
    print("\n\n######################## Sala de Chefe ########################\n")
    print(f"Ao olhar para frente, {player_nome} se depara com um grande portão de madeira entalhada com detalhes de ouro maciço.")
    print(f"É possivel notar uma grande aura detrás a porta e {player_nome} sente um arrepio em sua espinha.\n")
    print("1. Consultar Status do Jogador:\n")
    ...
    # Escolhe a próxima ação:
    while True:
        escolha_sala_boss = input("\nEscolha uma opcao: ")
        acao_sala_boss = opcoes_sala_boss.get(escolha_sala_boss)
